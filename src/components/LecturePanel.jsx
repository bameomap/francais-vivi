...
Tạo đúng ${cfg.q} câu hỏi. Câu hỏi và đáp án phải bằng tiếng Pháp. Giải thích (explain) bằng tiếng Việt.`;
}
      ...
      {mode === "ai" && <>
        ...
        {!lecture && !loading && (
          <div style={{ padding:"0.85rem 1rem" }}>
            <div style={{ background:C.white, borderRadius:18, padding:"1.5rem 1.2rem", border:`1.5px solid ${C.border}`, textAlign:"center", boxShadow:`0 2px 12px rgba(74,144,217,0.07)` }}>
              <div style={{ fontSize:"2.2rem", marginBottom:"0.5rem" }}>📰</div>
              <div style={{ fontFamily:"'Playfair Display',Georgia,serif", fontSize:"1.15rem", color:C.ink, fontWeight:700, marginBottom:"0.3rem" }}>Bài đọc hiểu</div>
              <div style={{ fontSize:"0.8rem", color:C.gray, marginBottom:"1.2rem", lineHeight:1.7 }}>
                {selectedUnit
                  ? <>AI tạo bài đọc theo chủ đề <b style={{ color:C.blue }}>{unitData?.title}</b></>
                  : "AI tạo đoạn văn từ bộ từ của bạn\nvà hỏi câu hỏi hiểu nội dung"
                }
              </div>
              ...
              <div style={{ display:"flex", gap:"0.5rem", marginBottom:"1.2rem", justifyContent:"center" }}>
                {Object.entries(LEVEL_CONFIG).map(([k, v]) => (
                  <button key={k} onClick={() => changeLevel(k)}
                    style={{ padding:"0.35rem 0.85rem", borderRadius:20, border:`1.5px solid ${level===k?C.blue:C.border}`, background:level===k?C.blueL:C.white, color:level===k?C.blue:C.gray, fontSize:"0.72rem", cursor:"pointer", fontWeight:600, transition:"all 0.15s" }}>
                    {v.label}
                  </button>
                ))}
              </div>
          ...
          <div style={{ padding:"0.75rem 1rem 4rem", display:"flex", flexDirection:"column", gap:"0.75rem" }}>
            ...
            <div style={{ display:"flex", gap:"0.4rem", flexWrap:"wrap", alignItems:"center" }}>
              ...
              <div style={{ display:"flex", gap:"0.3rem" }}>
                {Object.entries(LEVEL_CONFIG).map(([k, v]) => (
                  <button key={k} onClick={() => changeLevel(k)}
                    style={{ padding:"0.25rem 0.6rem", borderRadius:20, border:`1.5px solid ${level===k?C.blue:C.border}`, background:level===k?C.blueL:C.white, color:level===k?C.blue:C.gray, fontSize:"0.65rem", cursor:"pointer", fontWeight:600, transition:"all 0.15s" }}>
                    {v.label}
                  </button>
                ))}
              </div>
            ...
            {showText && (
              <div style={{ background:C.white, borderRadius:16, padding:"1.1rem 1.2rem", border:`1.5px solid ${C.border}`, boxShadow:"0 2px 12px #00000008" }}>
                ...
                {wordPopup && (
                  <div style={{ marginTop:"0.75rem", background:C.blueL, border:`1.5px solid ${C.blue}55`, borderRadius:14, padding:"0.75rem 1rem", animation:"fadeUp 0.2s ease", display:"flex", alignItems:"center", gap:"0.75rem" }}>
              ...
              {lecture.questions.map((q, qi) => {
                ...
                return (
                  <div key={qi} style={{ marginBottom:qi<lecture.questions.length-1?"1.1rem":0 }}>
                    ...
                    {done && q.explain && (
                      <div style={{ fontSize:"0.74rem", color:C.blue, background:C.blueL, borderRadius:10, padding:"0.45rem 0.7rem", marginTop:"-0.1rem" }}>
                        💡 {q.explain}
                      </div>
                    )}
                  </div>
            ...
            {allAnswered && (
              <div style={{ background:C.white, borderRadius:16, padding:"1.2rem", border:`1.5px solid ${pct>=80?"#059669":pct>=60?C.gold:C.red}44`, textAlign:"center", animation:"fadeUp 0.3s ease" }}>
                ...
                <button onClick={generate}
                  style={{ padding:"0.6rem 1.4rem", background:`linear-gradient(135deg, ${C.accent}, #c0392b)`, color:C.white, border:"none", borderRadius:12, fontSize:"0.85rem", cursor:"pointer", fontFamily:"'Playfair Display',Georgia,serif", fontWeight:700, boxShadow:`0 4px 14px ${C.accent}44` }}>
                  🔄 Bài mới
                </button>
              </div>
            )}
